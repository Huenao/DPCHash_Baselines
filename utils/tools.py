...
import cv2
from utils.CIB_data import GaussianBlur
...
def config_dataset(config):
    ...
    if config["dataset"] == "nuswide_21":
        config["data_path"] = "/data/gongqk/data/NUS-WIDE/"
    if config["dataset"] in ["nuswide_21_m", "nuswide_81_m"]:
        config["data_path"] = "/dataset/nus_wide_m/"
    if config["dataset"] == "coco":
        config["data_path"] = "/data/gongqk/data/coco/"
    if config["dataset"] == "voc2012":
        config["data_path"] = "/dataset/"
    if config["dataset"] == "imagenet":
        config["data_path"] = "/data/wangld/data/imagenet/"
    config["data"] = {
...
def image_transform(resize_size, crop_size, data_set):
    if data_set == "train_set":
        ...
        color_jitter = transforms.ColorJitter(0.4,0.4,0.4,0.1)
        return transforms.Compose([transforms.RandomResizedCrop(size = 224,scale=(0.5, 1.0)),
                                        transforms.RandomHorizontalFlip(),
                                        transforms.RandomApply([color_jitter], p = 0.7),
                                        transforms.RandomGrayscale(p  = 0.2),
                                        GaussianBlur(3),
...
def cifar_dataset(config):
    ...
    cifar_dataset_root = '/data/gongqk/data/cifar-10-batches-py'
    # Dataset
...
def get_data_CIB(config):
    ...
    train_loader = util_data.DataLoader(train_dataset,
                            batch_size=data_config["train_set"]["batch_size"],
                            shuffle=True, num_workers=4)
    test_loader = util_data.DataLoader(test_dataset,
                            batch_size=data_config["test"]["batch_size"],
                            shuffle=False, num_workers=4)
    database_loader = util_data.DataLoader(database_dataset,
...
def compute_result_feas(dataloader, net, device):
    bs, clses, feas = [], [], []
    net.eval()
    for img, cls, _ in tqdm(dataloader):
        clses.append(cls)
        b, fea = net(img.to(device))
        bs.append(b.data.cpu())
        feas.append(fea.data.cpu())
    return torch.cat(bs).sign(), torch.cat(clses), torch.cat(feas)

def compute_result_CIB(dataloader, net, device):
    bs, clses = [], []
    net.eval()
    for img, cls in tqdm(dataloader):
        clses.append(cls)
        bs.append((net.encode_discrete(img.to(device))).data.cpu())
    return torch.cat(bs).sign(), torch.cat(clses)

def compute_result_vit(dataloader, net, device):
    bs, clses = [], []
    net.eval()
    for img, cls, _ in tqdm(dataloader):
        clses.append(cls)
        hash_code = net(img.to(device))[1]
